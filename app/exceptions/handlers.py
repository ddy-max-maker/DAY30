...
日志策略（所有日志都通过 RequestIdFilter 自动带 request_id）：
- BusinessError（业务异常）：INFO 级别，不打印 traceback
  （业务异常属于预期内分支，不是程序错误）
- HTTPException：按 status_code 区分
  - 4xx（客户端错误）：WARNING
  - 5xx（服务端错误）：ERROR + traceback
- RequestValidationError（参数校验失败 422）：INFO
  （用户传错参数，不是程序错误）
- 兜底 Exception（未捕获异常）：ERROR + logger.exception 保留 traceback
"""
